...
public class DailyUsageService {
    ...
    public DailyUsage createOrUpdate(DailyUsageDTO dailyUsageDTO) {
        ...
        Date usageDate;
        System.out.println("dailyUsageDTO.getUsageDate() {}");
        System.out.println(dailyUsageDTO.getUsageDate());

        if (dailyUsageDTO.getUsageDate() != null) {
            // Adjust dailyUsage.UsageDate timezone and set it to midnight
            dailyUsageDTO.setUsageDate(
                    DateManilpulation.resetTimeToMidnight(dailyUsageDTO.getUsageDate()));
        ...
        if (currDailyUsage != null) {
            ...
            Number newUsage = dailyUsageDTO.getUsedInMb().longValue()
                    + currDailyUsage.getUsedInMb().longValue();
            ...
            if ((DateManilpulation.resetTimeToMidnight(currDailyUsage.getCycle().getEndDate())
                    .compareTo(usageDate) == 0 && currDailyUsage.getNextCycleId() == null)) {
                ...
                Cycle newCycle = this.cycleRepository.save(
                        new Cycle(dailyUsageDTO.getMdn(), startDate, dailyUsageDTO.getUserId()));
            ...
            DailyUsage newDailyUsage = this.dailyUsageRepository.save(
                    new DailyUsage(dailyUsageDTO.getMdn(), dailyUsageDTO.getUserId(), usageDate,
                            dailyUsageDTO.getUsedInMb(), null, previousUsage.getCycle()));
            logger.info("Created new daily usage for user [{}] and mdn [{}] with date [{}]",
                    dailyUsageDTO.getUserId(), dailyUsageDTO.getMdn(),
                    newDailyUsage.getUsageDate());
